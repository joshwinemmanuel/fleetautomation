...
def apply_common_filters(data_df):
    ...
    data_df = data_df[(data_df['Total mileage (km)'] > 20) &
                      (data_df['Autononous driving mileage (km)'] > 10)]
...
def process_data():
    ...
    for test in test_data_eu.keys():
        for hostname in test_data_eu[test]['Hostname'].unique():
            results = results.append({'Test': test, 'Hostname': hostname}, ignore_index=True)
